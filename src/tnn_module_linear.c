...
tnn_error tnn_module_bprop_linear(tnn_module *m){
  ...
  if((ret = tnn_numeric_v2m(&m->w.x, &w, m->output->size, m->input->size)) != TNN_ERROR_SUCCESS){
    return ret;
  }
  if((ret = tnn_numeric_v2m(&m->w.dx, &dw, m->output->size, m->input->size)) != TNN_ERROR_SUCCESS){
    return ret;
  }

  //bprop to input
  if(gsl_blas_dgemv(CblasTrans, 1.0, &w, &m->output->dx, 0.0, &m->input->dx) != 0){
    return TNN_ERROR_GSL;
  }

  //bprop to dw
  gsl_matrix_set_zero(&dw);
  if(gsl_blas_dger(1.0, &m->output->dx, &m->input->x, &dw) != 0){
    return TNN_ERROR_GSL;
  }
...
tnn_error tnn_module_fprop_linear(tnn_module *m){
  ...
  if(gsl_blas_dgemv(CblasNoTrans, 1.0, &w, &m->input->x, 0.0, &m->output->x) != 0){
    return TNN_ERROR_GSL;
  }
